...
```
 ...
 │  graphs    │   (password)   │  │ Queries │ config    │             │
 └────────────┘                │  │ on read │ (volume)  │             │
                               │  └─────────┴───────────┘             │
                               └──────────────────────────────────────┘
...
### 2. The Collector (Next.js API route, `POST /e`)
...
3. **Derive the visitor hash** — `sha256(ip + user_agent + site + daily_salt)`. The `daily_salt` is a random value rotated every 24h and never persisted beyond its day, so yesterday's hashes can't be reconstructed even from a DB dump. The raw IP and UA are used only in-memory for the hash and then discarded.
4. **Resolve session** — same visitor hash within a 30-minute sliding window = same session.
5. **Insert** a row into `events`.

No queue, no batching at v1 — at <100k events/month a synchronous SQLite insert is comfortably fast. (A write buffer is a later optimization, noted below.)
...
### 4. The Dashboard (Next.js + React)

Single-user, password-protected. Per site:

- Visitors and pageviews over time (line/area chart), with range selector (24h / 7d / 30d / custom).
...
- **One Next.js app, not separate collector + dashboard.** Single deploy, single language, lowest cost. *Rejected:* a standalone ingestion service for load isolation — premature at this scale; can be split later if a single site ever goes viral.
...
- **SQLite + Railway durability.** Data lives on a single persistent volume. Mitigation: a scheduled `VACUUM INTO` backup to object storage; SQLite's single-file nature makes backups trivial.
...
- [ ] Dashboard: visitors/pageviews over time, top pages, top referrers, device breakdown, current-visitors
- [ ] Per-site config: `autoTrack` flags + flat `goals` list, read by both tracker and dashboard
- [ ] Single-password auth
- [ ] Deploy to Railway: one service, SQLite on a persistent volume, backup job
